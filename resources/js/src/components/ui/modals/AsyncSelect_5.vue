<template>
    <div :class="[width]" class="flex flex-col bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-2xl select-none">
        ...
        <div class="relative h-[450px] bg-slate-900/50 group" @mousedown="initAudio" @touchstart="initAudio">

            <div
                @click="stepScroll(-1)"
                class="absolute top-4 left-1/2 -translate-x-1/2 z-30 p-2 cursor-pointer text-slate-500 hover:text-blue-400 transition-colors opacity-0 group-hover:opacity-100 active:scale-90"
            >
            ...
            >
                ...
                <div
                    v-for="item in items"
                    :key="item.id"
                    class="h-[50px] flex items-center justify-center px-6 snap-center snap-stop-always transition-all duration-300 cursor-pointer"
                    @click="scrollToItem(item.id)"
                >
                    <span
                        class="text-sm truncate transition-all duration-300 pointer-events-none"
                        :class="[
                            localSelectedId === item.id
                                ? 'text-blue-400 font-bold scale-125'
                                : 'text-slate-500 opacity-20 hover:opacity-50'
                        ]"
                    >
                        {{ item.name }}
                    </span>
            ...
            <div
                @click="stepScroll(1)"
                class="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 p-2 cursor-pointer text-slate-500 hover:text-blue-400 transition-colors opacity-0 group-hover:opacity-100 active:scale-90"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const playClick = () => {
    ...
    gain.gain.setValueAtTime(0.15, audioCtx.value.currentTime)
    gain.gain.exponentialRampToValueAtTime(0.001, audioCtx.value.currentTime + 0.04)
...
/* Принудительная остановка на каждом элементе */
.snap-center {

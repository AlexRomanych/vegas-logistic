                ...
                <div :class="[width, height, borderColor, 'modal-container ']">
                    ...
                    <div class="close-cross-container">
                        <div class="flex-1 flex items-center px-4">
                            <span v-if="localSelectedIds.size > 0" class="text-xs text-blue-400 font-bold">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-0.5 bg-slate-900/30">
                        <div
                            ...
                            class="group flex items-center justify-between px-4 py-2 rounded-lg cursor-pointer transition-all duration-200"
                            @click="toggleSelection(item.id)"
                        >
                            <div class="flex flex-col">
                                <span class="text-sm font-medium">
                                    {{ func ? func(item) : item.id.toString() }}
                                </span>
                                <span v-if="item.description"
                                      :class="localSelectedIds.has(item.id) ? 'text-blue-100' : 'text-slate-500'"
                            ...
                            <div
                                :class="localSelectedIds.has(item.id) ? 'bg-blue-500 border-blue-400' : 'border-slate-600 group-hover:border-slate-500'"
                                class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors">
                                <span v-if="localSelectedIds.has(item.id)" class="text-[10px]">✓</span>
                            </div>
                        </div>
...
interface IProps {
    items: T[]
    modelValue?: (number | string)[] | null // Изменено на массив
    // modelValue?: number | string | null
    title?: string
    width?: string
    height?: string
    type?: IColorTypes,
    func?: null | ((entity: T) => string)
}
...
// __ Используем Set для удобства работы с уникальными значениями
const localSelectedIds = reactive(new Set<number | string>())

const showModal = ref(false)
let resolvePromise: ((value: boolean) => void) | null

const show = async (initValues: (number | string)[] | null = []) => {
    await nextTick()

    localSelectedIds.clear()
    initValues?.forEach(id => localSelectedIds.add(id))
...
defineExpose({
    show,
    get selected() {
        console.log('filtered items: ', props.items.filter(i => localSelectedIds.has(i.id)))
        return props.items.filter(i => localSelectedIds.has(i.id))
    },
...
const filteredItems = computed(() => {
    ...
    return props.items
        .filter(item =>
            (props.func ?
                props.func(item).toLowerCase().includes(query) :
...
const handleConfirm = () => {
    ...
    const selectedItems = props.items.filter(i => localSelectedIds.has(i.id))
...
watch(() => props.modelValue, (newVal) => {
    localSelectedIds.clear()
    newVal?.forEach(id => localSelectedIds.add(id))

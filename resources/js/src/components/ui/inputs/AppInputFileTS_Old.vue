<template>
    ...
    <div class="w-full max-w-xl mx-auto p-4">
        <div v-if="!isUploading" class="flex items-center justify-center w-full">
            <label
                class="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50 hover:bg-slate-100 hover:border-indigo-400 transition-all duration-300 cursor-pointer group">
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p class="mb-2 text-sm text-slate-700 font-semibold text-center px-4">
                        Кликните для выбора JSON или перетащите файл
                    </p>
                    <p class="text-xs text-slate-500">JSON (до 10 MB)</p>
                </div>
                <input accept=".json" class="hidden" type="file" @change="handleFileChange"/>
            </label>
        </div>
        ...
        <div v-else class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm transition-all duration-500">
            ...
            <div v-if="progress < 100" class="mt-4 flex items-center justify-center space-x-2 animate-pulse">
...
const handleFileChange = (event) => {
    ...
    if (file.value) {
        emits('selectFile', e.target.files[0])
        // uploadFile()
    }
}

    ...
    <div v-if="!collapsed">
        <div v-for="entity of entitiesRender" :key="entity.id">
            <div class="flex ">
                ...
                <!-- __ ответственный -->
                <AppLabelTSWrapper
                    v-if="assemblyDay.responsible && entity.id === assemblyDay.responsible.id"
                    :arg="entity"
...
import type { IColorTypes, IRenderData, IAssemblyDay, IAssemblyDayWorker } from '@/types'
...
type IEntity = IAssemblyDayWorker
...
const { globalWorkers } = storeToRefs(assemblyStore)        // __ Все работники (Global State)

// __ Данные для отображения
const entitiesRender = computed(() => props.assemblyDay.workers)
...
const render: IRenderData = reactive({
    collapsed  : {
        ...
        headerType    : () => 'warning',
        dataType      : () => DATA_TYPE,
        type          : (entity: IEntity) => props.assemblyDay.responsible && entity.id === props.assemblyDay.responsible.id ? 'success' : 'danger',
        headerTextSize: HEADER_TEXT_SIZE,
        dataTextSize  : DATA_TEXT_SIZE,
        headerAlign   : HEADER_ALIGN,
        dataAlign     : 'center',
        placeholder   : '🔍collapsed...',
        data          : (entity: IEntity) => props.assemblyDay.responsible && entity.id === props.assemblyDay.responsible.id ? '✓' : '',
        class         : 'cursor-pointer',
    },
    id         : {
        id            : () => 'id-search',
        ...
        dataType      : () => DATA_TYPE,
        type          : (entity: IEntity) => props.assemblyDay.responsible && entity.id === props.assemblyDay.responsible.id ? 'success' : DEFAULT_TYPE,
        headerTextSize: HEADER_TEXT_SIZE,
        dataTextSize  : DATA_TEXT_SIZE,
        headerAlign   : HEADER_ALIGN,
        dataAlign     : 'center',
    ...
    responsible: {
        ...
        data          : (entity: IEntity) => props.assemblyDay.responsible && entity.id === props.assemblyDay.responsible.id ? 'ответственный' : '',
...
const getSelectedWorkerIds = () => props.assemblyDay.workers.map((worker: IEntity) => worker.id)
const selectedWorkerIds    = ref(getSelectedWorkerIds())
// const selectedWorkerIds                    = computed(() => props.assemblyDay.workers.map((worker: IEntity) => worker.id))
...
const addWorkersGroup = async (/*workers: IEntity[]*/) => {
    ...
    // if (!isToday(new Date(props.assemblyDay.action_at))) {
    //     return
    // }
    ...
    if (answer) {
        ...
        const result                            = await assemblyStore.addWorkersToAssemblyDay(props.assemblyDay.id, selectedWorkersIdsSaved)
...
const addWorker = async (worker: IEntity) => {
    ...
    if (answer) {
        ...
        const result         = await assemblyStore.addWorkerToAssemblyDay(props.assemblyDay.id, selectedWorker.id)
...
const removeWorker = async (worker: IEntity) => {
    ...
    const result = await assemblyStore.removeWorkerFromAssemblyDay(props.assemblyDay.id, worker.id)
...
const setResponsible = async (worker: IEntity) => {
    ...
    const result = await assemblyStore.addResponsibleToAssemblyDay(props.assemblyDay.id, worker.id)
...
onMounted(async () => {
    await assemblyStore.getActiveWorkers()
})

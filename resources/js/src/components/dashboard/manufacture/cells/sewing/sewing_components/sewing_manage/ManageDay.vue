        ...
        >
            <template #item="{ element, index }">
                <div
                    ...
                    @click.exact="selectSewingTask(element)"
                    @click.ctrl="goToOrderCard(element)"
                >
                    <ManageItem
...
const amountAndTimeTotalDay = computed(() => {
    ...
    props.day.forEach(sewingTask => {
        const amountAndTime = getSewingTaskAmountAndTime(sewingTask as ISewingTask)
...
// __ Переход в карточку Заявки
const goToOrderCard = async (sewingTask: ISewingTask) => {
    await router.push({ name: 'orders.card', params: { id: sewingTask.order.id } })
    return
}
...
const checkMove = (evt: DraggableHTMLElement) => {
    ...
    const movedElement = evt.draggedContext.element as ISewingTask
...
const finishDrag = async (evt: DraggableHTMLElement) => {
    ...
    const movedElement = evt.item._underlying_vm_ as ISewingTask
...
const actionDayMenu = async () => {
    ...
    if (result.menuItem === 1) {
        const setStatusesData = clearDay.flatMap(task => {
            ...
            if (isTaskStatusCreated(task as ISewingTask) && !isTaskAverage(task as ISewingTask)) {
        ...
        const diffsTask = getSewingTasksDiff(newOrders, clearDay as ISewingTask[])

        const result = await sewingStore.saveChanges(newOrders, clearDay as ISewingTask[])
    ...
    if (result.menuItem === 3) {
        const grouped = getSewingTasksGroupedByOrder(clearDay as ISewingTask[]) // __ Получаем массив массивов СЗ по одинаковым Заявкам

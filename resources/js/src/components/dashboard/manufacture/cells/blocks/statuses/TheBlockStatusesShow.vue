<template>
    <div v-if="!isLoading" class="ml-2 mt-2">
        ...
        <!-- __ Данные -->
        <div v-for="blockTaskStatus of blockTaskStatusesRender" :key="blockTaskStatus.id" class="ml-2 max-w-fit">
            <div class="flex ">

                <!-- __ id -->
                <AppLabelTSWrapper :arg="blockTaskStatus" :render-object="render.id"/>

                <!-- __ Название -->
                <AppLabelTSWrapper :arg="blockTaskStatus" :render-object="render.name"/>

                <!-- __ Порядковый номер -->
                <AppLabelTSWrapper :arg="blockTaskStatus" :render-object="render.position"/>

                <!-- __ Цвет (Picker) -->
                <AppRGBPickerModalTS v-model="blockTaskStatus.color" @confirm="saveBlockTaskStatusColor($event, blockTaskStatus)"/>

                <!-- __ Описание -->
                <AppLabelTSWrapper :arg="blockTaskStatus" :render-object="render.description"/>
...
import type {
    IRenderData, IBlockTaskStatusEntity,
} from '@/types'
...
const blockTaskStatuses       = ref<IBlockTaskStatusEntity[]>([])
const blockTaskStatusesRender = ref<IBlockTaskStatusEntity[]>([])
...
const render: IRenderData = reactive({
    id:          {
        ...
        dataType:       () => DATA_TYPE,
        type:           () => DEFAULT_TYPE,
        color:          (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.color,
        headerTextSize: HEADER_TEXT_SIZE,
        dataTextSize:   DATA_TEXT_SIZE,
        headerAlign:    HEADER_ALIGN,
        dataAlign:      'center',
        placeholder:    '🔍id...',
        data:           (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.id.toString()
    },
    name:        {
        id:             () => 'name-search',
        header:         ['Название', ''],
        ...
        data:           (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.name
    },
    position:        {
        id:             () => 'position-search',
        ...
        dataAlign:      'center',
        placeholder:    '🔍№...',
        data:           (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.position.toString()
    },
    color:       {
        id:             () => 'color-search',
        header:         ['Цвет', 'ярлычка'],
        ...
        dataAlign:      'center',
        placeholder:    '🔍Цвет...',
        data:           (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.color,
    ...
    description: {  // __ Описание Заявки
        ...
        data:           (blockTaskStatus: IBlockTaskStatusEntity) => blockTaskStatus.description ?? ''
    },
})
...
const getBlockTaskStatuses = async () => {
    blockTaskStatuses.value = await blockStore.getBlockTaskStatuses()
    blockTaskStatuses.value = blockTaskStatuses.value
        .map(blockTaskStatus => ({...blockTaskStatus, description: blockTaskStatus.description ?? ''}))
        .sort((a, b) => a.id - b.id)
}

// __ Формируем отображение Заявок
const getBlockTaskStatusesRender = () => {
    blockTaskStatusesRender.value = blockTaskStatuses.value
}

// __ Сохраняем данные по цвету
const saveBlockTaskStatusColor = async (event: string, blockTaskStatus: IBlockTaskStatusEntity) => {
    await blockStore.patchBlockTaskStatusColor(blockTaskStatus.id, event)
}
...
onMounted(async () => {
    ...
    await loaderHandler(
        ...
        async () => {
            ...
            // if (DEBUG) console.log('blockTaskStatuses: ', blockTaskStatuses.value)
            getBlockTaskStatusesRender()
            // if (DEBUG) console.log('blockTaskStatusesRender: ', blockTaskStatusesRender.value)
        },
        undefined,
        // false,
    )

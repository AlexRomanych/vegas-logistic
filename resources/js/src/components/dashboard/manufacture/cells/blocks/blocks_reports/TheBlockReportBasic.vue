                    ...
                    >
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <h3 :class="isDarkMode ? 'text-white' : 'text-slate-900'" class="text-xl font-black leading-none">Смена №{{ group.groupName }}</h3>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-6 text-base">
                            ...
                            <div :class="isDarkMode ? 'border-slate-800' : 'border-slate-300'" class="text-right border-l-2 pl-5">
                                ...
                                <span :class="isDarkMode ? 'text-emerald-400' : 'text-emerald-600'" class="font-black text-lg">{{ Number(group.square.total).toFixed(2) }} м²</span>
                            </div>
                            <!--<span-->
                        ...
                        <div v-for="(sub, sIdx) in group.subgroups" :key="sIdx"
                            ...
                            >
                                ...
                                <div :class="isDarkMode ? 'text-slate-400' : 'text-slate-500'" class="flex items-center gap-5 text-sm font-semibold">
                                    <div>
                                        <span>Кол-во: </span>
                                        <strong :class="isDarkMode ? 'text-indigo-400' : 'text-indigo-600'" class="text-base font-black">{{ sub.amount.total }}</strong>
                                        <span v-if="sub.amount.done !== undefined" :class="isDarkMode ? 'text-slate-500' : 'text-slate-400'" class="text-xs font-bold"> / {{ sub.amount.done }}</span>
                                        <span :class="isDarkMode ? 'text-slate-500' : 'text-slate-400'" class="text-xs font-normal"> шт.</span>
                                    </div>

                                    <div v-if="sub.square && sub.square.total" :class="isDarkMode ? 'border-slate-700/60' : 'border-slate-200'" class="border-l pl-4">
                                        <span>Площадь: </span>
                                        <strong :class="isDarkMode ? 'text-emerald-400' : 'text-emerald-600'" class="text-base font-black">{{ Number(sub.square.total).toFixed(2) }} м²</strong>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="!sub.collapsed"
                                ...
                                <table class="w-full text-left border-collapse text-sm">
                                    ...
                                    <tr v-for="line in sub.lines" :key="line.id"
                                        ...
                                        <td :class="isDarkMode ? 'text-slate-400' : 'text-slate-400'" class="py-1.5 px-3 font-mono font-bold text-base">#{{ line.position }}</td>
                                        <td class="py-1.5 px-3">
                                            <div class="flex items-center gap-3">
                                                <span :class="isDarkMode ? 'text-indigo-300 bg-indigo-950/60 border-indigo-900/50' : 'text-indigo-600 bg-indigo-50 border-indigo-100'"
                                                      class="font-mono font-black text-sm px-2 py-0.2 rounded border tracking-wide">
                                                    {{ line.block.code_1c }}
                                                </span>
                                                <span :class="isDarkMode ? 'text-white' : 'text-slate-900'" class="font-bold text-base">{{ line.block.name }}</span>
                                            </div>
                                        </td>
                                        <td :class="isDarkMode ? 'text-slate-100' : 'text-slate-900'" class="py-1.5 px-3 text-right font-black text-base">{{ line.amount }}</td>
                                        <td :class="isDarkMode ? 'text-emerald-400' : 'text-emerald-600'" class="py-1.5 px-3 text-right font-mono font-bold text-base">{{ Number(line.square * line.amount).toFixed(3) }}</td>
                                        <td :class="isDarkMode ? 'text-amber-400' : 'text-amber-600'" class="py-1.5 px-3 text-right font-mono font-bold text-base">{{ formatTimeWithLeadingZeros(line.time, 'hour') }}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>

<template>
    <div class="max-w-[750px] mx-auto my-8 flex flex-col gap-6 font-sans">
        <div class="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
            ...
            <div class="p-6">
                <div class="mb-5">
                    <label for="kdb-input" class="block mb-2 font-semibold text-slate-700 text-sm">
...
const isValid = computed(() => {
    return kdb.value.length > 0 && selectedFile.value !== null
})
...
const uploadDocument = async () => {
    ...
    const findExistItem = await sharedStore.getBlockDesignDocumentByKdb(kdb.value)
    if (findExistItem) {
        await showError([
            `КДБ ${kdb.value} уже загружено.`,
    ...
    formData.append('kdb', kdb.value.trim())
    formData.append('file', selectedFile.value)

    const result = await sharedStore.uploadBlockDesignDocument(formData)

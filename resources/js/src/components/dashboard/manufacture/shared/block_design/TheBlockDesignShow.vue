        ...
        <div class="sticky top-0 p-1 mb-1 bg-slate-200 border-2 rounded-lg border-slate-300 max-w">
            <div class="mx-0.5">
                <div class="flex">
                    ...
                    <div>
                        <!-- __ + КДБ -->
                        <router-link :to="{ name: 'manufacture.block.design.upload' }">
                            <AppLabelMultiLineTS
                                :list="entitiesRender"
                                :text="['➕', '']"
                                align="center"
...
import type { IColorTypes, IRenderData, IBlockDocument } from '@/types'
...
let entities: IBlockDocument[] = []
const entitiesRender           = ref<IBlockDocument[]>([])
...
const render: IRenderData = reactive({
    id  : {
        ...
        dataAlign     : 'center',
        placeholder   : '🔍Id...',
        data          : (doc: IBlockDocument) => doc.id.toString(),
        class         : 'cursor-pointer',
    },
    kdb: {
        id            : () => 'name-search',
        ...
        placeholder   : '🔍КДБ...',
        data          : (doc: IBlockDocument) => doc.kdb,
        class         : 'cursor-pointer',
    },
})
...
// __ Удаляем КДБ
const deleteDocument = async (blockDocument: IBlockDocument) => {
    modalInfoType.value = 'danger'
    ...
    const result = await sharedStore.deleteBlockDesignDocumentById(blockDocument.id)
    ...
    if (!checkCRUD(result)) {
        ...
        await appModalAsyncMultilineTS.value!.show()
        entitiesRender.value = entitiesRender.value.filter(doc => doc.id !== blockDocument.id)
        return
    }
}
...
const showDocument = async (blockDocument: IBlockDocument) => {
    doc.value = blockDocument
...
watchEffect(() => {
    ...
    const kdbSearch = kdbFilter.value.toLowerCase()
    const idSearch   = idFilter.value.toLowerCase()

    entitiesRender.value = entities
        .filter(entity => entity.kdb.toLowerCase().includes(kdbSearch))
        .filter(entity => entity.id.toString().toLowerCase().includes(idSearch))
})

// __ Получение данных
const getEntities = async () => {
    entities = await sharedStore.getBlockDesignDocuments()
    entities = entities
        .sort((a, b) => a.kdb.localeCompare(b.kdb))
}

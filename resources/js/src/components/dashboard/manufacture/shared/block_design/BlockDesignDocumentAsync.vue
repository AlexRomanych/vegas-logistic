                ...
                <div :class="[width, height, borderColor, 'modal-container p-4']">
                    ...
                    <div class="flex justify-between items-center w-full mb-2 h-auto">
                        <h3 :class="[currentTextColor, 'font-semibold text-lg']">
                            Просмотр документации КДБ №{{ doc?.kdb || '' }}
                        </h3>
                        <AppInputButton
                            id="close"
                            :type="type"
...
import type { IColorTypes, IBlockDocument } from '@/types'
...
interface IProps {
    ...
    mode?: 'inform' | 'confirm'
    okWord?: string
    doc: IBlockDocument | null | undefined
}
...
const loadPdfDocument = async () => {
    ...
    const result = await sharedStore.getBlockDesignDocumentByIdBlob(docData!.id)

<template>
    <div
        class="mr-1 bg-blue-950 rounded-xl border-2 border-blue-600 overflow-hidden shadow-[0_0_20px_rgba(37,99,235,0.2)] flex flex-col h-[450px]">

        <div v-if="title" class="px-5 py-3 bg-blue-600 flex items-center justify-between z-10 shrink-0">
        ...
        <div class="flex-1 overflow-hidden relative p-2">
            <div class="marquee-content hover:pause-animation">
                <div class="flex flex-col gap-1 pb-1">
                    <div
                        v-for="(item, index) in items"
                        :key="'a' + index"
                        class="flex items-center bg-blue-950/30 border border-blue-900 rounded-lg group"
                    >
                        <div class="w-48 flex-none bg-blue-900/50 px-3 py-1 border-r border-blue-800">
                            <span class="text-blue-400 text-[11px] font-black uppercase tracking-tight">{{ item.label }}</span>
                        </div>
                        <div class="bg-[#111c3a] flex-1 px-4 py-1">
                            <span class="text-white text-sm font-bold tracking-wide">{{ item.value || 'НЕТ ДАННЫХ' }}</span>
                        </div>
                    </div>
                </div>

...
import type { IColorTypes, IMaterialRenderGroup, IRenderOrder } from '@/types'
...
const actionText = computed(() => groups.value.length !== 0 ? 'Удалить СВПМ' : 'Создать СВПМ')
const actionType = computed(() => groups.value.length !== 0 ? 'danger' : 'success')
...
const actionTask = async () => {
    ...
    if (groups.value.length !== 0) {
        ...
        modalInfoText.value = [
            'Сводная ведомость потребности',
            'материалов будет удалена.',
            'Продолжить?',
        ]
        ...
        result = await ordersStore.deleteOrdersMaterials([props.id])
        ...
        result = await ordersStore.createOrdersMaterials([props.id])

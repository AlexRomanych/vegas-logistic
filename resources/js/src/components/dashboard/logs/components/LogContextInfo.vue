                ...
                <div :class="[width, height, borderColor, 'modal-container']">
                    ...
                    <div class="w-full pl-8 border-b border-slate-800/50">
                        <h3 class="text-left text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-1">
                            {{ eventLog ? getModuleTitle(eventLog.target) : '' }}
                        </h3>
                    ...
                    <div class="p-4 max-h-[60vh] overflow-y-auto custom-scrollbar space-y-1">
                        <div
                            v-for="(item, index) in shortData"
                            :key="index"
                            class="flex items-center px-4 py-2 rounded-lg hover:bg-[#161e2d] transition-colors group"
                        >
                            <span
                                class="w-[120px] shrink-0 text-slate-500 text-[11px] font-semibold tracking-tight"
                            >
                                {{ item.label }}
                            </span>
                            <span
...
import type { IColorTypes, IEventLog } from '@/types'
...
import { getModuleTitle } from '@/app/helpers/helpers_logs.ts'
...
const title = computed(() => props.eventLog?.message || '')
...
const shortData = computed(() => {
    if (props.eventLog && props.eventLog.context) {
        return Object.entries(props.eventLog.context).map(([label, value]) => ({
            label,
            value,
        }))
        // const result = []
        // for (const [label, value] of Object.entries(props.material.properties)) {
...
const longData = computed<Record<string, string>[]>(() => [])
...
.modal-leave-to {
    opacity: 0;
    transform: scale(1.1); /* Легкое увеличение для эффекта приближения */
}
</style>

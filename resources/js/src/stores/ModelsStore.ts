...
const URL_MODELS_PROCEDURE         = 'models/procedure'          // URL для получения определенной процедуры расчета с сервера по коду 1С
...
// const URL_MODELS_CONSTRUCT         = 'models/construct/'         // URL для получения спецификаций моделей с сервера
const URL_MODELS_CONSTRUCT_MODEL   = 'models/construct/model'    // URL для получения спецификации конкретной модели с сервера
...
export const useModelsStore = defineStore('models', () => {
    ...
    //__ Получаем с API список отдельную процедур расчета
    const getProcedure = async (code_1c: string | null = null) => {
        if (!code_1c) return null

        const response = await jwtGet(`${URL_MODELS_PROCEDURE}/${code_1c}`)
        const result   = await response

        if (DEBUG) console.log('ModelsStore: getProcedure: ', result)
        return result.data
    }
    ...
    const getConstructByModelCode1c = async (model_code_1c: string | null = null) => {
        if (!model_code_1c) return null

        const response = await jwtGet(`${URL_MODELS_CONSTRUCT_MODEL}/${model_code_1c}`)
        const result   = await response

        if (DEBUG) console.log('ModelsStore: getConstructByModelCode1c: ', result)
        return result.data
    }


    //__ Получаем с API Спецификацию по коду спецификации
    const getConstructByCode1c = async (code_1c: string | null = null) => {
        if (!code_1c) return null

        const response = await jwtGet(`${URL_MODELS_CONSTRUCTS}/${code_1c}`)
        const result   = await response
    ...
    return {
        ...
        getConstructs,
        getConstructByModelCode1c, getConstructByCode1c,
        uploadConstructs,
    }
})

...
class AssemblyDay extends Model
{
     ...
     * @param $date
     * @param int $change
     * @return AssemblyDay|Model
     */
    public static function findOrCreateByDateAndChange($date, int $change = 1): AssemblyDay|Model
    {
        return self::query()
    ...
    public function workers(): BelongsToMany
    {
        return $this
            ->belongsToMany(
                ...
                AssemblyDayWorkerPivot::TABLE,       // Промежуточная Таблица, связывающая классы
                'assembly_day_id',                   // Ключ в промежуточной таблице, связывающий с текущим классом
                'worker_id'           // Ключ в промежуточной таблице, связывающий с классом, с которым связываемся
            )
            ->using(AssemblyDayWorkerPivot::class)

 ...
 */
class CheckCuttingTasksForDatesMiddleware
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        try {
            ...
            $cuttingTasksBefore = CuttingTask::query()
                ...
                ->sort(function ($a, $b) use ($statusOrder) {
                    ...
                    $statusA = $a->latestTaskStatusByDate->cutting_task_status_id;
                    $statusB = $b->latestTaskStatusByDate->cutting_task_status_id;
            ...
            if ($cuttingTasksBefore->count() > 0) {

                // __ Получаем задачи на текущий день
                $cuttingTasksNow = CuttingTask::query()
                ...
                // __ 0. В cuttingTasksBefore дату action_at заменить на текущую
                // __ 1. При равных данных приоритет у коллекции cuttingTasksBefore (элементы будут стоять выше при равных условиях)
                ...
                $cuttingTasksBefore->transform(function ($item) use ($nowDate) {
                ...
                $cuttingTasksNow->transform(function ($item) {
                ...
                $combinedTasks = $cuttingTasksBefore->concat($cuttingTasksNow)
                    ->sort(function ($a, $b) use ($statusOrder) {
                        // __ Условие 2: Сортировка по статусу (в указанном порядке)
                        $posA = array_search($a->latestTaskStatusByDate->cutting_task_status_id, $statusOrder);
                        $posB = array_search($b->latestTaskStatusByDate->cutting_task_status_id, $statusOrder);
                ...
                // __ Обновляем CuttingTasks текущего дня

...
return new class extends Migration
{
    ...
    private const TABLE_NAME = 'block_task_status_pivot';
    ...
    public function up(): void
    {
        Schema::create(self::TABLE_NAME, function (Blueprint $table) {
            ...
            $table->foreignId('block_task_id')
                ->nullable(false)
                ->comment('Связь с СЗ')
                ->constrained('block_tasks', 'id')
                ->cascadeOnDelete();

            $table->foreignId('block_task_status_id')
                ->nullable(false)
                ->comment('Связь со статусом')
                ->constrained('block_task_statuses', 'id')
                ->cascadeOnDelete();
            ...
            // $table->unique(['block_task_id', 'block_task_status_id']);

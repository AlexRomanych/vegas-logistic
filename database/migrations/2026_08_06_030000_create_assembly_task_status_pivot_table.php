...
return new class extends Migration
{
    ...
    private const TABLE_NAME = 'assembly_task_status_pivot';
    ...
    public function up(): void
    {
        Schema::create(self::TABLE_NAME, function (Blueprint $table) {
            ...
            $table->foreignId('assembly_task_id')
                ->nullable(false)
                ->comment('Связь с СЗ')
                ->constrained('assembly_tasks', 'id')
                ->cascadeOnDelete();

            $table->foreignId('assembly_task_status_id')
                ->nullable(false)
                ->comment('Связь со статусом')
                ->constrained('assembly_task_statuses', 'id')
                ->cascadeOnDelete();
            ...
            // $table->unique(['assembly_task_id', 'assembly_task_status_id']);
